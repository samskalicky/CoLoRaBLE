...
class BLEmanager: NSObject, ObservableObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    let nodeCharacteristicUUID = CBUUID.init(string: "d233a9d8-f33e-4e3b-be6c-52914e5947fe")
    ...
    func connectTo(peripheral: CBPeripheral) {
        ...
        if data!.loraMap[nodeID] == nil {
            ...
            data!.node = node
            data!.loraMap[nodeID] = data!.nodes.last
        }
        data!.node!.isSelf = true
        ...
        if let services = peripheral.services {
            for service in services {
                if service.uuid == serviceUUID {
                    ...
                    peripheral.discoverCharacteristics([msgCharacteristicUUID, nodeCharacteristicUUID, loraCharacteristicUUID, radioCharacteristicUUID], for: service)
                }
            }
        }
    }
        ...
        if let characteristics = service.characteristics {
            for characteristic in characteristics {
                if let periph = data!.periphMap[peripheral.identifier.uuidString] {
                    ...
                    } else if characteristic.uuid == nodeCharacteristicUUID {
                        periph.nodeInfo = characteristic
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        if let periph = data!.periphMap[peripheral.identifier.uuidString] {
            ...
            if characteristic == periph.msg {
                ...
                let str = NSString(data: val, encoding: String.Encoding.utf8.rawValue)! as String
                loraStr += str
                if str.last! == String(UnicodeScalar(31)!).last! {
                    loraStr.removeLast()
                    readLora(peripheral: peripheral)
                } else {
                    data!.updateNetwork(info: loraStr)
                    loraStr = ""
                }
            } else if characteristic == periph.radio {
                let str = NSString(data: val, encoding: String.Encoding.utf8.rawValue)! as String
                print("radio: "+str)
            } else if characteristic == periph.nodeInfo {
                let str = NSString(data: val, encoding: String.Encoding.utf8.rawValue)! as String
                data!.info = data!.parseNodeInfo(info: str)
                data!.deviceGPS = String(format: "%3.5f, %3.5f",data!.info!.position.latitude, data!.info!.position.longitude)
            }
        }
    }
    ...
    // async request to read lora radio info
    ...
    // async request to read node info
    func readNodeInfo(peripheral: CBPeripheral) {
        if let periph = data!.periphMap[peripheral.identifier.uuidString] {
            if periph.isConnected {
                if let nodeInfo = periph.nodeInfo {
                    peripheral.readValue(for: nodeInfo)
                }
            }
        }
    }
